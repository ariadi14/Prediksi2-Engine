    ...
    def fetch(self, fixture):
        ...
        for p in self.providers:
            ...
            try:
                ...
                attempts.append({'provider':name,'ok':bool(raw.get('fixture_match') or raw.get('predictions') or raw.get('odds')),'errors':raw.get('errors',[])})
                if raw.get('fixture_match'): merged['fixture_match']=raw['fixture_match']
                for k,v in raw.items():
                    if k not in ('provider','fixture_match','errors') and v not in (None,{},[]): merged.setdefault(k,v)
