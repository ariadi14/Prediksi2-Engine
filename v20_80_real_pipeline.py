...
from v20_80_data_preparation import prepare_evidence, normalize_name
from v20_80_model_audit import external_model_probabilities, adaptive_weights, ensemble_1x2
...
def diagnose_pipeline_health(counts: Dict[str, int], unresolved: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    elif resolved > 0 and window == 0:
        reasons.append("NO_FIXTURE_IN_SELECTED_WINDOW")
    ...
    return {
        ...
        "message": (
            ...
            if status == "HEALTHY_NO_QUALIFIED_MARKETS"
            else ("Fixture berhasil di-resolve tetapi tidak ada yang masuk time window manual yang dipilih."
                  if status == "NO_FIXTURES_IN_SELECTED_WINDOW" and resolved > 0 else None)
        ),
    }
def main() -> int:
    ...
    ap.add_argument("--output", default="artifacts/v20_80_final_output.json")
    args = ap.parse_args()
    ...
    evidence_attempts = 0
    resolved_outside_window = []

    for fixture in fixtures:
        if not fixture.get("home") or not fixture.get("away"):
            unresolved.append({"fixture": fixture, "reason": "MISSING_TEAM_NAME"})
            continue
        if normalize_name(fixture.get("home")) == normalize_name(fixture.get("away")):
            unresolved.append({"fixture": fixture, "reason": "OCR_SELF_MATCH_REJECTED"})
            continue
        ...
        if not in_window(resolved_kickoff, args.time_window):
            resolved_outside_window.append({
                "fixture": f"{resolved.get('home_canonical') or resolved.get('home')} vs {resolved.get('away_canonical') or resolved.get('away')}",
                "kickoff": resolved_kickoff,
                "match_date": resolved.get("match_date"),
                "fixture_date_resolution_mode": resolved.get("fixture_date_resolution_mode"),
            })
            continue
        ...
        ev_payload = prepare_evidence(ev_payload, resolved)
        ...
        # V20.80 external-model gate: activate external probabilities only
        # after a real settled audit sample; otherwise preserve V20.79.
        external = external_model_probabilities(ev_payload)
        metrics = ev_payload.get("external_model_metrics", {})
        baseline_metrics = ev_payload.get("v20_80_baseline_metrics", {})
        metric_pack = {"v20_79_baseline": baseline_metrics if isinstance(baseline_metrics, dict) else {}}
        if isinstance(metrics, dict):
            metric_pack.update(metrics)
        weights = adaptive_weights(metric_pack, minimum_settled=20)
        eligible_external = {}
        if isinstance(metrics, dict):
            for name, probs in external.items():
                m = metrics.get(name, {})
                if isinstance(m, dict) and int(m.get("settled", 0) or 0) >= 20:
                    eligible_external[name] = probs
        ens = ensemble_1x2(prediction.get("markets", {}).get("1X2", {}), eligible_external, weights)
        prediction["v20_80_ensemble"] = {
            "status": ens.get("status"),
            "weights": ens.get("weights"),
            "eligible_external_models": sorted(eligible_external),
            "audit_metrics": metric_pack,
        }
        if ens.get("status") == "ADAPTIVE_ENSEMBLE":
            prediction["markets"]["1X2"] = ens["probabilities"]
    ...
    counts = {
        ...
        "resolved_outside_selected_window": len(resolved_outside_window),
        "ocr_self_match_rejections": sum(1 for u in unresolved if u.get("reason") == "OCR_SELF_MATCH_REJECTED"),
    }
    engine_diagnostic = diagnose_pipeline_health(counts, unresolved)

    output = {
        "engine_version": "V20.80",
        "baseline": "V20.79",
        "source": "PelangiEuro",
        ...
        "fixture_search_date_mode": "AUTO_JAKARTA_TODAY_NEARBY_INFERRED" if str(args.fixture_date).upper() == "AUTO" else "EXPLICIT",
        ...
        "resolved_outside_window": resolved_outside_window,
        "v20_80_plan": {
            "data_preparation": True,
            "external_model_audit": True,
            "adaptive_ensemble": True,
            "external_activation_requires_settled_audit": 20,
            "v20_79_baseline_preserved": True,
        },
